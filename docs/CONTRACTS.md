...
Fixture notes (from H0):
- Fixtures are generated by `docs/contracts/make_fixtures.py`. Edit the script and regenerate; don't hand-edit the JSON.
- `state_stabilising.json` has `severity.state = "CRITICAL"` with `pending = {kind: "stepdown", to: "STABILISING"}`. A step-down is only *proposed* while still in CRITICAL/EMERGENCY and happens after IC confirmation. The file name reflects the pending target, not the current state.
- `state_emergency.json` is the standalone "no reduce-only" branch (PLAN §3.9), not part of the main idle → resolved sequence. The stub reaches it via `POST /incident/inject`.
