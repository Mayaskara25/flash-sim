...
| `insurance_fund_usd` | 75000.0 | Starting simulated fund. C1 no-action crosses 25% around T+20 while reduce-only at T+18 holds near 42% |
| `slippage` | 0.20 | **Execution-depth factor**, not a fee or a per-tick multiplier: `fill = liq` moved adverse by `slippage × |cumulative drawdown|`. Shallow early moves create little bad debt; the sharper T+11–14 leg drains the fund |
| `liq_capacity_per_min` | 360 | Maximum queue executions per minute. Default `null` is unbounded for other scenarios. Unused capacity never rolls over, and queued positions execute at the then-current price |
...
calm market never escalates); `LIQ_RATE` = rolling-60 s scaled executed
count (= per-minute rate); `L_exp` = the same queue's rolling executed count
without the `LAR_MULT` fault. `LAR = L_obs / L_exp` (1.0 below counting
significance — fewer than 10 observed liquidations/min; denominator floored
at 2/min against book-tail lumpiness). This avoids mistaking a legitimate
queue backlog during a price bounce for a liquidation-engine fault.
`BAD_DEBT_RATE` = rolling-60 s shortfall as % of start
fund, driven by the depth-based slip above so it turns on during Warning
and accelerates with the deeper second leg; `ADL_COUNT` increments
...
`L_exp` in the running signal generator is the same executed queue count
without `LAR_MULT`; LAR stays near 1 through a throughput-bound market move,
including a price bounce while the backlog drains. The static
`expected_per_min` price-threshold helper remains available for analysis.
...
- T+0..T+11: `LIQ_RATE` crosses 100/min by T+1 and stays at 100–290/min
  through T+11. The rules remain WARNING at minute markers T+1 through T+11;
  the gentle price leg avoids early CRITICAL. `PX_CHG_5M <= -5` at T+3 is
  deliberately **not** a target.
- T+6: `TICKET_RATE` ≈ 4× (I1); the fund is still near 100%, with bad debt
  first appearing later in the WARNING phase.
- T+11–14: the price falls to −11% and `LIQ_RATE` reaches 350–500/min;
  fund is ~35–45% at T+14. A rapid bounce to about −3.1% by T+14:10
  lets the liquidation queue continue through T+18 without immediately
  draining the fund below 25%. LAR stays market-explained.
- No-action: second waterfall T+18–22 drains the fund through 25% around
  T+20 (SEV-1 branch via the fund hard override; the fund does not recover
  on its own).
  ...
  the fund floors around 42% (never below 25%), `LIQ_RATE` < 50/min well
