...
export function heroRank(action: ActionView, pendingConfirm: boolean): number {
  if (action.control_id && action.what_if) {
    // A control the forecast says no longer helps (e.g. reduce-only once the
    // fund is already gone) must not be the hero.
    const w = action.what_if
    const helps = w.p_sev1_15_after < w.p_sev1_15_before - 0.05 || w.fund_p50_15_after > w.fund_p50_15_before + 1
    return helps ? 0 : 4
  }
