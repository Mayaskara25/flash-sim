...
export function SummaryView({ summary, templates, mock }: { summary: IncidentSummary | null; templates: TemplateView[]; mock: boolean }) {
  ...
  return <main className="min-h-screen bg-bg p-4 text-ink md:p-6"><div className="mx-auto max-w-5xl">
    ...
    <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border border-line bg-white p-4"><div className="text-sm"><strong>{summary.scenario_id}</strong> · peak SEV-{summary.peak_sev} · {simLabel(summary.started_t)} to {summary.ended_t == null ? 'ongoing' : simLabel(summary.ended_t)}</div><div className="flex items-center gap-2"><span role="status" className="text-xs text-muted">{copyStatus}</span>{!mock && <a href="/api/incident/report.pdf" target="_blank" rel="noreferrer" className="border border-navy px-3 py-2 text-xs font-semibold text-navy">Generate PDF report</a>}<button type="button" onClick={() => void copy()} className="bg-navy px-3 py-2 text-xs font-semibold text-white">Copy as Markdown</button></div></div>
