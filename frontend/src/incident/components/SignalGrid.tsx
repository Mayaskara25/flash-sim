...
export function SignalGrid({ signals, alerts, limit = 6, onMore }: { signals: SignalView[]; alerts: AlertCard[]; limit?: number; onMore?: () => void }) {
  ...
  return <section aria-label="Active signals" className="border border-line bg-white p-3 md:p-4">
    <div className="mb-3 flex items-baseline justify-between gap-2"><h2 className="text-xs font-bold uppercase tracking-[0.13em] text-ink">Active signals</h2><span className="text-[11px] text-muted">{signals.length} monitored · {primary.length} shown</span></div>
    ...
    {rest.length > 0 && <div className="mt-3 border-t border-line pt-2">{onMore
      ? <button type="button" onClick={onMore} className="text-xs font-semibold text-navy underline underline-offset-2">+{rest.length} more · open in Details › Signals</button>
      : <><button type="button" className="text-xs font-semibold text-navy underline underline-offset-2" aria-expanded={showMore} onClick={() => setShowMore((value) => !value)}>{showMore ? 'Hide' : 'Show'} {rest.length} more signals</button>
