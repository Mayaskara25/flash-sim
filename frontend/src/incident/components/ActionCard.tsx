...
export function ActionCard({ action, busy, onDecide }: { action: ActionView; busy: boolean; onDecide: (decision: 'approve' | 'skip', rationale: string) => Promise<void> }) {
  ...
  return <article className={`border border-line bg-white p-3 ${submitting ? 'opacity-55' : ''}`}>
    ...
    <p className="mt-2 text-[13px] font-medium leading-snug text-ink">{action.text}</p>
