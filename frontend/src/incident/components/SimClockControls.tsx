...
export function SimClockControls({ state, scenarios, actions, busy }: { state: IncidentStateDTO; scenarios: ScenarioSummary[]; actions: IncidentActions; busy: boolean }) {
  ...
  return <div className="flex flex-wrap items-center gap-2">
    ...
    {!state.sim.started ? <button type="button" disabled={busy} onClick={() => void actions.start({ scenario_id: scenarioId, speed: 8 }).catch(() => {})} className="border border-white bg-white px-3 py-1.5 text-xs font-semibold text-slate-900 hover:bg-slate-100 disabled:opacity-50">Start {scenarioId}</button> : <>
